...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("☜︎︎︎ 𝗕𝗮𝗰𝗸", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"𖣘 𝗣𝗮𝗴𝗲 {round(int(offset) / 10) + 1} / {round(total / 10)}",
                                  callback_data="pages")]
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"𖣘 𝗣𝗮𝗴𝗲 {round(int(offset) / 10) + 1} / {round(total / 10)}", callback_data="pages"),
             InlineKeyboardButton("𝗡𝗲𝘅𝘁 ☞︎︎︎", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("☜︎︎︎ 𝗕𝗮𝗰𝗸", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"𖣘 𝗣𝗮𝗴𝗲 1/{round(int(total_results) / 10)}", callback_data="pages"),
             InlineKeyboardButton(text="𝗡𝗲𝘅𝘁 ☞︎︎︎", callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="𖣘 𝗣𝗮𝗴𝗲 1/1", callback_data="pages")]
        )
    ...
    if imdb:
        ...
        cap = f"<b><i>[𝐌𝐨𝐯𝐢𝐞 𝐧𝐚𝐦𝐞](https://telegra.ph/file/1c4c77e0a3cf46a04e64c.jpg) : {search}\n 𝗥𝗲𝗾𝘂𝗲𝘀𝘁𝗲𝗱 𝗕𝘆 : {message.from_user.mention}\n𝗴𝗿𝗼𝘂𝗽 : {message.chat.title}</i></b>"
    if imdb and imdb.get('https://telegra.ph/file/1c4c77e0a3cf46a04e64c.jpg'):
        try:
            await message.reply_photo(photo="https://telegra.ph/file/1c4c77e0a3cf46a04e64c.jpg", caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
